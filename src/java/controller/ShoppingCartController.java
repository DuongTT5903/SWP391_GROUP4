...
protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    float total = 0;
    int totalPeople = 0;  // Variable to store total number of people
    ...
    for (Cart c : cartItems) {
        if (c != null && c.isCheckService()) {  // Check if the Cart object is not null and is selected
            ...
            if (c.getService() != null) {  // Ensure the service is not null
                ...
                // Add number of people (assuming 'getNumberOfPeople' returns the number of people for each service)
                totalPeople += c.getGetNumberOfPerson();  // Add the number of people from each checked service
            }
        }
    }
    ...
    request.setAttribute("totalPeople", totalPeople);  // Pass totalPeople to the JSP page
